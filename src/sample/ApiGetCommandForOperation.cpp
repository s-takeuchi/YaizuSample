				...
				if (OpCmd == Id[Loop]) {
					...
					*Ptr = L'\0';
					...
					for (int LoopAgt = 0; LoopAgt < 5; LoopAgt++) {
						CommandObj->AppendChildElement(new StkObject(L"AgentFileName", AgentFileName[Loop][LoopAgt]));
					}
					CommandObj->AppendChildElement(new StkObject(L"Timeout", Timeout[Loop]));
